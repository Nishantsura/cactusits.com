...
export default function ContactsAdmin() {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      setError(
        err.message || "An error occurred while fetching contact submissions",
      );
  ...
  const handleQuickStatusUpdate = async (id: number, newStatus: string) => {
    ...
    try {
      const { error } = await updateContactSubmission(id, {
        status: newStatus,
      });
  ...
  useEffect(() => {
    ...
    if (userTypeFilter !== "all") {
      results = results.filter(
        (submission) => submission.user_type === userTypeFilter,
      );
    }
