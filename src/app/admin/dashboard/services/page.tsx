          ...
          {services.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.map((service) => (
                <div key={service.id} className="border border-gray-200 rounded-lg overflow-hidden bg-white hover:shadow-md transition-shadow">
                  {/* Service card header */}
                  ...
                  <div className="p-4">
                    <div className="mb-3">
                      ...
                      <span className="text-gray-700 ml-2 text-sm font-mono">{service.slug}</span>
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      <p className="text-gray-700 text-sm mt-1">
                        {formatExcerpt(service.short_description || service.description, 120)}
                      </p>
                    </div>
                    ...
                    {service.icon_name && (
                      <div className="mb-3">
                        ...
                        <span className="text-gray-700 ml-2 text-sm font-mono">{service.icon_name}</span>
                      </div>
                    )}
                    ...
                    {service.hero_title && (
                      <div className="mb-3">
                        <span className="text-gray-500 text-sm">Hero Title:</span>
                        <p className="text-gray-700 text-sm mt-1">{formatExcerpt(service.hero_title, 80)}</p>
                      </div>
                    )}
                    ...
                    {service.image && (
                      <div className="mb-3">
                        <span className="text-gray-500 text-sm">Has Image:</span>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                        ...
                        try {
                          const bulletpoints = service.hero_bulletpoints ? 
                            (typeof service.hero_bulletpoints === 'string' ? 
                              JSON.parse(service.hero_bulletpoints) : service.hero_bulletpoints) : [];
                          return bulletpoints && Array.isArray(bulletpoints) && bulletpoints.length > 0 ? (
                            <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">Hero Content</span>
                          ) : null;
                        } catch (e) {
                          return null;
                        }
                      ...
                      {service.potential_description && (
                        <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">Potential Section</span>
                      )}
                      
                      {/* Check for explore section content */}
                      {service.hero_description && (
                        <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs rounded-full">Explore Section</span>
                      )}
                    </div>
