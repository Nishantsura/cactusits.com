...
async function main() {
  ...
  try {
    ...
    // Use a more direct approach to avoid PromiseLike type issues
    let error: any = null;
    try {
      await supabase
        .from("services")
    ...
    if (checkError) {
      console.log(
        "Table may not exist yet, attempting to create tables directly...",
      );
      // Let's try creating the tables directly
      // Use a more direct approach with try/catch to avoid PromiseLike type issues
      let createErrorValue: any = null;
      try {
        await supabase.from("execute_ddl").select("*").limit(1);
      } catch (err) {
        createErrorValue = err;
      }
      const createError = createErrorValue;

      if (createError) {
        console.warn(
          "Direct table creation not possible. You may need to create tables manually using Supabase SQL editor.",
        );
        console.warn(
          "Please refer to the SQL in supabase_tables.sql to create the required tables.",
        );
...
async function createSqlFunction() {
  // Use a different approach to create functions since rest is protected
